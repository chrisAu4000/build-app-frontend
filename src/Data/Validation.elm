...
includesString str val =
    if String.contains str val then
        Res val
    else
        Err [ "This input must contain '" ++ str ++ "'." ]
...
minLength n val =
    if String.length val < n then
        Err [ "This input must have a minimum length of " ++ toString n ++ "." ]
    else
        Res val
...
maxLength n val =
    if String.length val > n then
        Err [ "This input must have a maximum length of " ++ toString n ++ "." ]
    else
        Res val
...
includesIntegral val =
    if Regex.contains (regex "[0-9]") val then
        Res val
    else
        Err [ "This input must contain at least one number." ]
...
equals a b =
    if a == b then
        ...
        Err [ "Password and Verification must be equal." ]
...
lengthEquals n s =
    if String.length s == n then
        ...
        Err [ "This input must have a length of " ++ toString n ++ "." ]
...
isInt a =
    case String.toInt a of
        Result.Err _ ->
            Err [ "Only digits are allowed here." ]
