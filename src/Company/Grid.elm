    ...
    ShowRemoveDisclaymer companyId ->
      case (companyId, model.companies) of
        (Just companyId, RemoteData.Success companies) ->
          let
            equalIds = isEqual companyId
            newCompanies =
              List.map (\(_, com) -> 
                case com.id of
                  Nothing -> (False, com)
                  Just companyId ->
                    if equalIds companyId
                    then (True, com)
                    else (False, com)
              ) companies
          in
  ...
  let
    ...
    companyRef =
      company.id
        ...
        |> Maybe.withDefault []
    disclaymer = if showsDisclaymer then " show-disclaymer" else ""
    overlay = if showsDisclaymer then removeOverlay company else []
  in
    div
      [ class "relative overflow-hidden company grid-item sm-col sm-col-12 md-col md-col-4 lg-col lg-col-3" ]
      [ div
...
grid companies =
  case companies of
    ...
    RemoteData.Success cms -> List.map gridItem cms
