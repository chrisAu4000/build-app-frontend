...
import Html.Attributes exposing (autocomplete, class, for, id, required, style, type_, value)
...
errorMessage msgs =
    div
        [ class "error-message-wrapper shadow"
        , style [ ( "top", ("calc(" ++ calcErrorTop (List.length msgs) ++ " - 50px)") ++ "rem" ) ]
        ]
        [ ul
            [ class "error-message p1 list-reset" ]
            (List.map (\msg -> li [] [ text msg ]) msgs)
        , div
            [ class "box-arrow shadow" ]
            []
        ]
