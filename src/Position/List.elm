...
import Component.Button exposing (editBtn, deleteBtn)
    ...
    OnRemovedPosition webPosition ->
      case webPosition of
        RemoteData.Failure err -> 
          ...
          in
            ( model, Cmd.none )
        RemoteData.Success id -> ( removePosition id model, Cmd.none)
        _ -> ( model, Cmd.none )
    RequestsMsgs msg ->
      case msg of
...
renderListItem positionId position = 
  case positionId of
    Just id -> 
      if position.id == id 
      then selectedListItem position
      else unselectedListItem position
    Nothing -> listItem position
...
maybeList response =
  case response of
    ...
    RemoteData.Success positions -> list positions
