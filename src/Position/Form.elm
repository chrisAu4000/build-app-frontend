module Position.Form exposing (Model, Msg, init, initNew, update, view)

import Component.Button exposing (submitBtnFailure, submitBtnLoading, submitBtnNotAsked, submitBtnSuccess)
import Component.Input exposing (labeledTextArea, labeledTextInput)
import Html exposing (Attribute, Html, div, form, text)
import Html.Attributes exposing (class, required, value)
...
btnStateFailure =
    Failure "Sorry... Can't save Position"


btnStateSuccess : WebButton String
btnStateSuccess =
    Success "Yey Saved Position"
...
positionForm attrs children =
    let
        attributes =
            attrs ++ [ class "clearfix m1 position form" ]
    in
    div
